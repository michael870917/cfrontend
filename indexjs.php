<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
	<title>Armandola michel</title>
	<link rel="stylesheet" href="css/normalize.css">
	<link rel="stylesheet" href="css/main.css">
	<script src="js/jquery-1.11.1.min.js"></script>
	
</head>
<body>
	<header id="header" class="header">
		<div class="logo">
			<img src="images/logo.png" alt="Puls4">
		</div>
		<div class="titular">
			<h1 class="titulo">
				Puls4: Comunidad profesional de gente atractiva
			</h1>
			<div>
				<a class="filtro" href="#">
					Stylus
				</a>
				<a class="publicar" href="#">
					Publicar
				</a>				
			</div>
			<div>
				<a href="#14" class="sizer">A</a>
				<a href="#16" class="sizer">A</a>
				<a href="#18" class="sizer">A</a>
			</div>
		</div>
		<div class="usuario">
			<figure class="avatar">
				<img src="images/avatar.jpg" alt="Michael">
			</figure>
			<a class="flechita" href="#"></a>
		</div>

	</header><!-- /header -->
	<nav>
		<ul class="menu">
			<li><a href="#">Python</a></li>
			<li><a href="#">HTML5</a></li>
			<li><a href="#">Javascript</a></li>
			<li><a href="#">CSS3</a></li>
			<li><a href="#">Django</a></li>

		</ul>
	</nav>
	<form name="agreeArt" id="agreeArt">
		<!--p*3>label>{Titulo}+^input-->
		<p>
			<label for="titulo">Titulo</label>
			<input type="text" name="titulo" id="titulo" placeholder="Ingresa el titulo">
		</p>
		<p>
			<label for="autor">Autor</label>
			<input type="text" name="autor" id="autor" placeholder="Ingresa el autor">
		</p>
		<p>
			<label for="tag">tag</label>
			<input type="text" name="tag" id="tag" placeholder="Ingresa el tag">
		</p>
		<input type="submit" value="Publicar">
	</form>
	<section class="posts">
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">546</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">343</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">123</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">987</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">657</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
		<article class="post">
			<div class="descripcion">
				<figure class="imagen">
					<img src="images/foto.jpg">
				</figure>
				<div class="detalles">
					<h2 class="titulo">
						Colores, gradientes y texto 3D con Sass 7 Compass
					</h2>
					<p class="autor">
						por <a href="#">Aldair vallejo</a>
					</p>
					<a class="tag">PHP</a>
					<p class="fecha">hace <strong>20 min</strong></p>
				</div>
			</div>
			<div class="acciones">
				<div class="votos">
					<a href="#" class="up"></a>
					<span class="total">342</span>
					<a href="#" class="down"></a>
				</div>
				<div class="datos">
					<a class="comentarios" href="#">
						10
					</a>
					<a class="estrellita" href="#">
					</a>
				</div>

			</div>

		</article>
	</section>
	<footer>
		<h3>
			<strong>
				Powered by
			</strong>
			<span class="name_autor">
				michael vallejo	
			</span>
		 </h3>	
	</footer>

	<script type="text/javascript" src="js/controles.js"></script>
</body>
</html>